...
#
#   DESCRIPTION:  Runs Cartpole on multi-node GPU cluster.
#
...
#SBATCH --output logs/slurm-cartpole-container.log
...
python src/validation/nodes.py "logs/slurm-cartpole-container.log" 3
...
singularity exec \
    ...
    $CONTAINER_TGT_PATH \
    python -u /domi/syndeo/tests/python/cartpole_test.py $HEAD_NODE_ADDR
